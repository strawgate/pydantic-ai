...
description: "Explore one cross-cutting capability's support across all providers and file an issue for concrete parity gaps. Runs on the Pydantic AI harness engine; the prompt is iterable from a Logfire managed variable."
# Fuzzy weekly schedule (gh-aw scatters the time, auto-adds workflow_dispatch).
# Rotates one capability per run.
    ...
    steps:
      - name: Check out the default prompt file
        ...
        with:
          ...
          sparse-checkout: .github/workflows/shared/prompts/pydantic-ai-provider-parity-explore.md
      ...
      - name: Resolve agent prompt (Logfire managed variable, else committed default)
        ...
        env:
          ...
          LOGFIRE_VARIABLE_KEY: gh_aw_pydantic_ai_provider_parity_explore_prompt
          TARGETING_KEY: gh-aw-${{ github.repository }}
          DEFAULT_PROMPT_FILE: .github/workflows/shared/prompts/pydantic-ai-provider-parity-explore.md
        run: |
          set -euo pipefail
