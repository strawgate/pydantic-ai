...
| Built-in web tools | **Core:** `WebSearch` and `WebFetch` where supported; otherwise retain the application integration | Assert result shape, citations/events consumed by callers, credentials, and egress policy. |
...
| `allowed_tools`, `disallowed_tools`, permission modes, `can_use_tool` | **Core:** tool preparation, approval-required tools, deferred results; optional **Harness** policy; **Application:** authorization/UI/audit | Assert availability and each pre-effect allow, deny, or ask outcome independently, including source callback precedence. |
